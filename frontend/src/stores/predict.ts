...
export interface PredictResult {
  ...
  ensemble: { trend: string; confidence: number; votes: Record<string, number>; price_target: Record<string, number> } | { error: string }
}
...
export const usePredictStore = defineStore('predict', () => {
  ...
  async function load(code: string, days = 5) {
    loading.value = true
    error.value = ''
    try {
      result.value = await stockApi.predict(code, days)
    } catch (e: any) {
